...
export class LanguagesController {
    ...
    async getAll(@Res() res): Promise<Response> {
        try {
            this.logger.debug(`getAll: Obteniendo lenguajes poeditor`, {
            ...
            this.logger.debug(`getAll: catch error  ${e.message}`, {
                context: LanguagesController.name,
            });
    ...
    async getTermsLanguage(@Res() res, @Param('code') code: string): Promise<Response> {
        try {
            this.logger.debug(`getTermsLanguage: Obteniendo terminos del lenguaje:  [${code}]`, {
                context: LanguagesController.name,
            });
            ...
            this.logger.debug(`getTermsLanguage: catch error  ${e.message}`, {
                context: LanguagesController.name,
            });
            return res.status(HttpStatus.NOT_FOUND).send({ messages: 'Catch error' });
        }
